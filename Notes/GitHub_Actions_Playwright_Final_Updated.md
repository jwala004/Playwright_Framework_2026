**🔵 DOCUMENT UPDATE — September 2026**

This version has been updated to reflect the **current implemented architecture**.

**Current baseline:** QA / UAT / PROD execution, GitHub Environment-based CI configuration, `BASE_URL` + `APP_USERNAME` + `APP_PASSWORD` stored as Environment Secrets, manual environment selection, scheduled PROD execution, Node.js 24 LTS runtime governance through `package.json` + `.npmrc`, and npm caching in GitHub Actions.

**Change markers used in this document:**
- 🟢 **[NEW]** — newly added content
- 🔵 **[UPDATED]** — existing content updated to match the current implementation
- 🟡 **[CLARIFIED]** — existing concept clarified
...
Missing required environment variable "APP\_PASSWORD"
...
APP\_USERNAME=qa\_user

APP\_PASSWORD=qa\_password

.env.uat

BASE\_URL=https\://uat.example.com

APP\_USERNAME=uat\_user

APP\_PASSWORD=uat\_password

.env.prod

BASE\_URL=https\://prod.example.com

APP\_USERNAME=prod\_user

APP\_PASSWORD=prod\_password
...
    "test\:qa": "cross-env TEST\_ENV=qa playwright test",

    "test\:qa\:headed": "cross-env TEST\_ENV=qa playwright test --headed",

    "test\:uat": "cross-env TEST\_ENV=uat playwright test",

    "test\:uat\:headed": "cross-env TEST\_ENV=uat playwright test --headed",

    "test\:prod": "cross-env TEST\_ENV=prod playwright test",

    "test\:prod\:headed": "cross-env TEST\_ENV=prod playwright test --headed"
...
${{ secrets.APP\_USERNAME }}

${{ secrets.APP\_PASSWORD }}
...
17\. Manual Execution with workflow\_dispatch
...
GitHub supports choice inputs for workflow\_dispatch.
...
  name: ${{ github.event\_name == 'schedule' && 'PROD' || inputs.environment }}
...
  BASE\_URL: ${{ vars.BASE\_URL }}

  APP\_USERNAME: ${{ secrets.APP\_USERNAME }}

  APP\_PASSWORD: ${{ secrets.APP\_PASSWORD }}
...
process.env.APP\_USERNAME

process.env.APP\_PASSWORD
...
🟢 **[NEW] npm dependency caching**

The workflow uses `cache: 'npm'` in `actions/setup-node`. This caches npm package data to improve dependency installation performance without replacing `npm ci` with a potentially stale `node_modules` directory.
...
Mistake 3 — Using GitHub Variables for sensitive values

Incorrect:

APP\_PASSWORD: ${{ vars.APP\_PASSWORD }}
...
Mistake 5 — Assuming workflow\_dispatch means scheduled execution
...
BASE\_URL configuration problem
...
if [ -n "$APP\_USERNAME" ]; then

  echo "APP\_USERNAME: available"

else

  echo "APP\_USERNAME: MISSING"
...
I externalize environment-specific configuration using environment files locally and GitHub Environment variables/secrets in CI. A TEST\_ENV variable determines the target environment, and a centralized TypeScript configuration module loads and validates the appropriate configuration.
...
Manual executions use a GitHub Actions workflow\_dispatch choice input for QA, UAT, or PROD. Scheduled executions automatically map to PROD.

Q: How do you prevent invalid environments?

Answer:

I maintain an allowedEnvironments constant using TypeScript as const, derive a union type from it, and validate TEST\_ENV before loading configuration.
...
5\. Use workflow\_dispatch for manual execution
...
# 🔵 FINAL UPDATE CHANGE LOG — What Changed and Why

### 1. 🔵 `env.config.ts` — Updated
**What changed:** Documented the current environment validation, dynamic `.env.<environment>` selection, dotenv loading, required-variable validation, and typed `config` export.

**Why:** The notes must describe the current implementation and its fail-fast configuration behavior.

### 2. 🔵 GitHub Environment Secrets — Updated
**What changed:** `BASE_URL`, `APP_USERNAME`, and `APP_PASSWORD` are now documented as Environment Secrets for QA, UAT, and PROD.

**Why:** This matches the current implementation. It also avoids the previous mismatch where `BASE_URL` was documented under `vars`.

### 3. 🔵 Local vs CI configuration — Updated
**What changed:** Local execution is documented as `.env.<environment>` → dotenv → `process.env`; CI is documented as GitHub Environment Secrets → workflow environment variables → `process.env`.

**Why:** This demonstrates the same test code working with two different configuration sources.

### 4. 🔵 Manual + scheduled execution — Updated
**What changed:** The documentation continues to distinguish manual QA/UAT/PROD selection from automatic PROD scheduling.

**Why:** This is the framework's intended operational model.

### 5. 🟢 Node.js 24 LTS governance — Added
**What changed:** Added:
- `package.json` → `node: >=24.0.0 <25.0.0`
- `package.json` → `npm: >=10.0.0`
- `.npmrc` → `engine-strict=true`

**Why:** Establishes an explicit and enforceable runtime compatibility contract.

### 6. 🔵 GitHub Actions Node setup — Corrected to the actual implementation
**What changed:** The CI configuration is documented as:
...
**Why:** Your workflow reads the Node.js requirement from `package.json`, making it the single source of truth rather than duplicating the version in YAML.

### 7. 🟢 npm caching — Added
**What changed:** Documented `cache: 'npm'`.

**Why:** It improves dependency installation performance by caching npm package data while preserving clean `npm ci` installation.

### 8. 🟡 `lts/*` approach — Clarified
**What changed:** The notes no longer present `node-version: 24` as your actual workflow implementation.

**Why:** Your actual implementation uses `node-version-file: 'package.json'`.

### 9. ❌ Email notifications — Not marked as implemented
**Why:** Email reporting was intentionally postponed.

### 10. ❌ Allure / test-history trend analysis — Not marked as implemented
**Why:** These remain future enhancements until actually configured.
